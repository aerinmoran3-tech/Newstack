...
export async function up(db: any) {
  await db.execute(sql`
    -- Creates a stored procedure that inserts a property and its photos atomically
...
export async function down(db: any) {
  await db.execute(sql`DROP FUNCTION IF EXISTS create_property_with_photos(jsonb, text[]);`);
}
